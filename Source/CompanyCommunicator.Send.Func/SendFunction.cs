...
namespace Microsoft.Teams.Apps.CompanyCommunicator.Send.Func
{
    ...
    public class SendFunction
    {
        ...
        private readonly ILogger<SendFunction> log;
        ...
        private async Task<IMessageActivity> GetMessageActivity(SendQueueMessageContent message, string recipientId, ILogger log)
        {
            var notification = await this.notificationRepo.GetAsync(
                NotificationDataTableNames.SendingNotificationsPartition,
                message.NotificationId);

            log.LogInformation($"BaseUrl : {this.optionsRepository.Value.AppBaseUrl} -> Message Content : {notification.Content}");

            string strToSearch = $"{this.optionsRepository.Value.AppBaseUrl}/api/ReadReceiptNotification/view";
            log.LogInformation($"String to search  : {strToSearch}");
            if (notification.Content.ToLower().Contains(strToSearch.ToLower()))
            {
                string modifiedString = $"{strToSearch}?id={notification.NotificationId}&rowKey={recipientId}";
                modifiedString = modifiedString.Replace("azurefd.net", "azurewebsites.net");
                notification.Content = notification.Content.Replace(strToSearch, modifiedString);
                log.LogInformation($"modified string : {modifiedString} - > Message Content Replaced : {notification.Content}");
            }
        ...
        private async Task<bool> SendEmailTeamplate(SendQueueMessageContent messageContent, ILogger log)
        {
            ...
            try
            {
                ...
                if (notification.TemplateType.ToUpper() == "UPLOAD EMAIL TEMPLATE" && (notification.SendTypeId == "2" || notification.SendTypeId == "3" || notification.SendTypeId == "4" || notification.SendTypeId == "6"))
                {
                    Microsoft.Graph.User user = null;
                    IUsersServiceByTenant userServiceByTenant = null;
                    if (!string.IsNullOrEmpty(messageContent.RecipientData.UserData.TenantId))
                    {
                        userServiceByTenant = this.graphServiceFactoryByTenant.GetUsersService(messageContent.RecipientData.UserData.TenantId);
                        user = await userServiceByTenant.GetUserAsync(messageContent.RecipientData.UserData.AadId);
                    }
                    else
                    {
                        user = await this.usersService.GetUserAsync(messageContent.RecipientData.UserData.AadId);
                    }
                    ...
                    if (!string.IsNullOrEmpty(htmlContent))
                    {
                        ...
                        {
                            sentResult = await userServiceByTenant.SendMailToUserAsync(notification.CreatedBy, notification.Title, user.Mail, htmlContent, true);
                        }
                        else
                        {
                            sentResult = await this.usersService.SendMailToUserAsync(notification.CreatedBy, notification.Title, user.Mail, htmlContent, true);
                        }
            ...
            try
            {
                if (!string.IsNullOrEmpty(notification.ImageLink))
                {
                    ...
                    if (!string.IsNullOrEmpty(fileName))
                    {
                        ...
                        CloudStorageAccount storageAccount = Microsoft.Azure.Storage.CloudStorageAccount.Parse(this.optionsRepository.Value.StorageAccountConnectionString);
