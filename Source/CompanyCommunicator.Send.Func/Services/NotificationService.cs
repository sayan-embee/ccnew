...
#pragma warning disable SA1124 // Do not use regions
        #region Recall Notification

        /// <inheritdoc/>
        public async Task<bool> IsSendRecallNotificationThrottled()
        {
            var globalNotificationStatus = await this.globalSendingNotificationDataRepository.GetGlobalSendingRecallNotificationDataEntityAsync();
            if (globalNotificationStatus?.SendRetryDelayTime == null)
            {
                return false;
            }
        ...
        /// <inheritdoc/>
        public async Task<bool> IsPendingRecallNotification(SendRecallQueueMessageContent message)
        {
            var recipient = message?.RecipientData;
            if (string.IsNullOrWhiteSpace(recipient?.RecipientId))
            {
            ...
            // Check notification status for the recipient.
            var notification = await this.recallSentNotificationDataRepository.GetAsync(
                partitionKey: message.NotificationId,
                rowKey: message.RecipientData.RecipientId);
            ...
            // 2. FaultedAndRetryingStatusCode: this means the Azure Function previously attempted to send the notification
            //    to this recipient but failed and should be retried.
            if (notification?.StatusCode == RecallSentNotificationDataEntity.InitializationStatusCode ||
                notification?.StatusCode == RecallSentNotificationDataEntity.FaultedAndRetryingStatusCode)
            {
                return true;
            }
        ...
        /// <inheritdoc/>
        public async Task UpdateSentRecallNotification(
            string notificationId,
            string recipientId,
            int totalNumberOfSendThrottles,
            int statusCode,
            ...
            string activityId)
        {
            ...
            if (statusCode == (int)HttpStatusCode.Created)
            {
                notification.DeliveryStatus = RecallSentNotificationDataEntity.Succeeded;
            }
            else if (statusCode == (int)HttpStatusCode.TooManyRequests)
            {
                notification.DeliveryStatus = RecallSentNotificationDataEntity.Throttled;
            }
            else if (statusCode == (int)HttpStatusCode.NotFound)
            {
                notification.DeliveryStatus = RecallSentNotificationDataEntity.RecipientNotFound;
            }
            else if (statusCode == RecallSentNotificationDataEntity.FaultedAndRetryingStatusCode)
            {
                notification.DeliveryStatus = RecallSentNotificationDataEntity.Retrying;
            }
            else
            {
                notification.DeliveryStatus = RecallSentNotificationDataEntity.Failed;
            }

            await this.recallSentNotificationDataRepository.InsertOrMergeAsync(notification);
        }
        #endregion
