...
import { selectMessage, getMessagesList, getDraftMessagesList,getRecalledMessagesList } from '../../actions';
...
class Overflow extends React.Component<OverflowProps, OverflowState> {
                    ...
                    items: [
                        {
                            ...
                            onClick: (event: any) => {
                                ...
                                let url = getBaseUrl() + "/viewstatus/" + this.props.message.id + "?locale={locale}&openfrom=send";
                        ...
                        {
                            key: 'recall',
                            content: this.localize("Recall"),
                            onClick: (event: any) => {
                                event.stopPropagation();
                                this.setState({
                                    menuOpen: false,
                                });
                                let url = getBaseUrl() + "/recallconfirmation/" + this.props.message.id + "?locale={locale}";
                                this.onOpenTaskModuleRecallConfirmation(null, url, this.localize("RecallConfirmation"));
                            }
                        },
                    ],
                },
    ...
    private onOpenTaskModuleRecallConfirmation = (event: any, url: string, title: string) => {
        let taskInfo: ITaskInfo = {
            url: url,
            title: title,
            height: 530,
...
export default connect(mapStateToProps, { selectMessage, getMessagesList, getDraftMessagesList, getRecalledMessagesList})(overflowWithTranslation);
