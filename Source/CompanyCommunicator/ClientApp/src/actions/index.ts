...
import { getSentNotifications, getDraftNotifications, getScheduledNotifications, getRecallSentNotifications } from '../apis/messageListApi';
...
export const getRecalledMessagesList = () => async (dispatch: any) => {
    const response = await getRecallSentNotifications();
    const notificationList: Notification[] = response.data;
    notificationList.forEach(notification => {
        notification.sendingStartedDate = formatDate(notification.sendingStartedDate);
        notification.sentDate = formatDate(notification.sentDate);
    });
    dispatch({ type: 'FETCH_RECALLEDMESSAGES', payload: notificationList });
};