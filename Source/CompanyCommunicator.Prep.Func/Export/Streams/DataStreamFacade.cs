...
namespace Microsoft.Teams.Apps.CompanyCommunicator.Prep.Func.Export.Streams
{
    ...
    public class DataStreamFacade : IDataStreamFacade
    {
        ...
        public async IAsyncEnumerable<IEnumerable<UserData>> GetUserDataStreamAsync(string notificationId, string tenantId)
        {
            ...
            await foreach (var sentNotifcations in sentNotificationDataEntitiesStream)
            {
                ...
                try
                {
                    ...
                    {
                        var usersServiceByTenant = this.graphServiceFactoryByTenant.GetUsersService(tenantId);

                        // filter the recipient not found users.
                        var users = await usersServiceByTenant.GetBatchByUserIds(
                            sentNotifcations
