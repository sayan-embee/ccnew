...
namespace Microsoft.Teams.Apps.CompanyCommunicator.Prep.Func.Export.Activities
{
    ...
    public class GetMetadataActivity
    {
            ...
            ExportDataEntity exportDataEntity) exportRequiredData)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(exportRequiredData.notificationDataEntity.TenantId))
                {
                    var usersServiceByTenant = this.graphServiceFactoryByTenant.GetUsersService(exportRequiredData.notificationDataEntity.TenantId);
                    user = await usersServiceByTenant.GetUserAsync(exportRequiredData.exportDataEntity.PartitionKey);
                }
                else
                {
