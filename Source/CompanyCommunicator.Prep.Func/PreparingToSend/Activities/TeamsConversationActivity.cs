...
namespace Microsoft.Teams.Apps.CompanyCommunicator.Prep.Func.PreparingToSend
{
    ...
    public class TeamsConversationActivity
    {
            ...
            ILogger log)
        {
            ...
            if (input.recipient.TenantId == null)
            {
                throw new ArgumentNullException(nameof(input.recipient.TenantId));
            }
            ...
            {
                recipient.ServiceUrl = await this.appSettingsService.GetServiceUrlAsync(recipient.TenantId);
            }

            var messageContent = JsonConvert.SerializeObject(recipient);
            log.LogInformation($"Receipient Details : {messageContent}");
        ...
        private async Task<string> InstallAppAndGetConversationId(
            ...
            ILogger log)
        {
            var appId = await this.appSettingsService.GetUserAppIdAsync(recipient.TenantId);
            ...
            try
            {
                var appManagerServiceByTenant = this.graphServiceFactoryByTenant.GetAppManagerService(recipient.TenantId);
                await appManagerServiceByTenant.InstallAppForUserAsync(appId, recipient.RecipientId);
            ...
            catch (ServiceException exception)
            {
                switch (exception.StatusCode)
                {
                    ...
                    case HttpStatusCode.NotFound:
                        ...
                        await this.appSettingsService.DeleteUserAppIdAsync(recipient.TenantId);
                        return string.Empty;
                    default:
            ...
            try
            {
                var chatsServiceByTenant = this.graphServiceFactoryByTenant.GetChatsService(recipient.TenantId);
                return await chatsServiceByTenant.GetChatThreadIdAsync(recipient.RecipientId, appId);
