...
namespace Microsoft.Teams.Apps.CompanyCommunicator.Prep.Func.PreparingToSend
{
    ...
    public class SyncGroupMembersActivity
    {
        ...
        [ActivityTrigger](string notificationId, string groupId, string tenantId) input, ILogger log)
        {
            ...
            {
                throw new ArgumentNullException(nameof(input.tenantId));
            }
            ...
            try
            {
                ...
                var grpMemberService = this.graphServiceFactoryByTenant.GetGroupMembersService(tenantId);
                var users = await grpMemberService.GetGroupMembersAsync(groupId);

                // Convert to Recipients
                var recipients = await this.GetRecipientsAsync(notificationId, users, tenantId);
        ...
        private async Task<IEnumerable<SentNotificationDataEntity>> GetRecipientsAsync(string notificationId, IEnumerable<User> users, string tenantId)
        {
