    ...
    .ConfigureServices((context, services) =>
    {
        ...
        services.AddOptions<BotOptions>()
            .Configure<IConfiguration>((opt, cfg) =>
            {
                ...
                opt.AuthorAppId = cfg.GetValue<string>("AuthorAppId");
                opt.AuthorAppPassword = cfg.GetValue<string>("AuthorAppPassword");
            });

        services.AddOptions<DataQueueMessageOptions>()
            .Configure<IConfiguration>((opt, cfg) =>
            {
                opt.MessageDelayInSeconds = cfg.GetValue<double>("DataQueueMessageDelayInSeconds", 5);
            });

        services.AddOptions<Microsoft.Teams.Apps.CompanyCommunicator.Prep.Func.TeamsConversationOptions>()
            .Configure<IConfiguration>((opt, cfg) =>
            {
                opt.ProactivelyInstallUserApp = cfg.GetValue<bool>("ProactivelyInstallUserApp", true);
                opt.MaxAttemptsToCreateConversation = cfg.GetValue<int>("MaxAttemptsToCreateConversation", 2);
            });
        ...
        services.AddScoped<IGraphServiceFactoryByTenant, GraphServiceFactoryByTenant>();
