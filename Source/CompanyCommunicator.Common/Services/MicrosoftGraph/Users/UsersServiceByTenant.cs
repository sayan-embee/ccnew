// <copyright file="UsersServiceByTenant.cs" company="Microsoft">
...
namespace Microsoft.Teams.Apps.CompanyCommunicator.Common.Services.MicrosoftGraph
{
    ...
    /// Users service.
    /// </summary>
    public class UsersServiceByTenant : IUsersServiceByTenant
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="UsersServiceByTenant"/> class.
        /// </summary>
        /// <param name="graphServiceClient">graph service client.</param>
        internal UsersServiceByTenant(IGraphServiceClient graphServiceClient)
        {
            this.graphServiceClient = graphServiceClient ?? throw new ArgumentNullException(nameof(graphServiceClient));
        }

        /// <inheritdoc/>
        public async Task<IEnumerable<User>> GetBatchByUserIds(IEnumerable<IEnumerable<string>> userIdsByGroups, string type = "ID")
        {
            ...
            var batches = this.GetBatchRequest(userIdsByGroups, type);
        ...
        private string GetUserIdFilter(IEnumerable<string> userIds, string type = "ID")
        {
            ...
            foreach (var id in userIds)
            {
                ...
                if (type == "EMAIL")
                {
                    filterUserIds.Append($"mail eq '{id}'");
                }
                else
                {
        ...
        private IEnumerable<BatchRequestContent> GetBatchRequest(IEnumerable<IEnumerable<string>> userIdsByGroups, string type = "ID")
        {
            ...
            foreach (var userIds in userIdsByGroups)
            {
                ...
                var filterUserIds = this.GetUserIdFilter(userIds, type);
                var httpRequestMessage = new HttpRequestMessage(HttpMethod.Get, $"https://graph.microsoft.com/v1.0/users?$filter={filterUserIds}&$select=id,displayName,userPrincipalName,mail");
