...
namespace Microsoft.Teams.Apps.CompanyCommunicator.Common.Services.Teams
{
    ...
    public class MessageService : IMessageService
    {
            ...
            await this.botAdapter.ContinueConversationAsync(
                ...
                callback: async (turnContext, cancellationToken) =>
                {
                    ...
                    try
                    {
                        ...
                        response.ActivityId = message.Id;
        ...
        /// <inheritdoc/>
        public async Task<SendMessageResponse> UpdateMessageAsync(
            IMessageActivity message,
            string conversationId,
            string serviceUrl,
            int maxAttempts,
            ILogger log)
        {
            ...
            await this.botAdapter.ContinueConversationAsync(
                ...
                callback: async (turnContext, cancellationToken) =>
                {
                    ...
                    {
                        // Send message.
                        await policy.ExecuteAsync(async () => await turnContext.UpdateActivityAsync(message));
                        response.ActivityId = message.Id;
                        response.ResultType = SendMessageResult.Succeeded;
                        response.StatusCode = (int)HttpStatusCode.Created;
                        response.AllSendStatusCodes += $"{(int)HttpStatusCode.Created},";
                    ...
                    {
                        var errorMessage = $"{e.GetType()}: {e.Message}";
                        log.LogError(e, $"Failed to recall message. Exception message: {errorMessage}");
        ...
        /// <inheritdoc/>
        public async Task<SendMessageResponse> DeleteMessageAsync(
            string conversationId,
            string activityId,
            string serviceUrl,
            int maxAttempts,
            ILogger log)
        {
            ...
            await this.botAdapter.ContinueConversationAsync(
                ...
                callback: async (turnContext, cancellationToken) =>
                {
                    ...
                    {
                        // Send message.
                        await policy.ExecuteAsync(async () => await turnContext.DeleteActivityAsync(conversationReference));
