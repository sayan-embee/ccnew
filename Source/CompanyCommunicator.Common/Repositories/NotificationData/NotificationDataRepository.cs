...
namespace Microsoft.Teams.Apps.CompanyCommunicator.Common.Repositories.NotificationData
{
    ...
    public class NotificationDataRepository : BaseRepository<NotificationDataEntity>, INotificationDataRepository
    {
        ...
        {
            string filter1 = TableQuery.GenerateFilterConditionForBool("IsRecalled", QueryComparisons.NotEqual, true);

            var result = await this.GetWithFilterAsync(filter1, NotificationDataTableNames.SentNotificationsPartition, 20);
        ...
        public async Task<string> MoveDraftToSentPartitionAsync(NotificationDataEntity draftNotificationEntity)
        {
            try
            {
                ...
                var sentNotificationEntity = new NotificationDataEntity
                {
                    ...
                    TemplateType = draftNotificationEntity.TemplateType,
                    SendTypeId = draftNotificationEntity.SendTypeId,
                    TenantId = draftNotificationEntity.TenantId,
                    AdaptiveCardContent = draftNotificationEntity.AdaptiveCardContent,
                    EmailTitle = draftNotificationEntity.EmailTitle,
                    EmailBody = draftNotificationEntity.EmailBody,
                    CsvLink = draftNotificationEntity.CsvLink,
                    AdditionalFileLink = draftNotificationEntity.AdditionalFileLink,
                    TenantName = draftNotificationEntity.TenantName,
                    AuthorTeamId = draftNotificationEntity.AuthorTeamId,
                    AuthorTeamName = draftNotificationEntity.AuthorTeamName,
                    AuthorChannelId = draftNotificationEntity.AuthorChannelId,
                    AuthorChannelName = draftNotificationEntity.AuthorChannelName,
                    TotalReadReceipt = 0,
                };
        ...
        /// <inheritdoc/>
        public async Task<string> MoveSentToRecallPartitionAsync(NotificationDataEntity sendNotificationEntity)
        {
            try
            {
                if (sendNotificationEntity == null)
                {
                    throw new ArgumentNullException(nameof(sendNotificationEntity));
                }

                // var newSentNotificationId = this.TableRowKeyGenerator.CreateNewKeyOrderingMostRecentToOldest();

                // Create a recall notification based on the sent notification.
                var sentNotificationEntity = new NotificationDataEntity
                {
                    PartitionKey = NotificationDataTableNames.RecallSentNotificationsPartition,
                    RowKey = sendNotificationEntity.RowKey,
                    Id = sendNotificationEntity.RowKey,
                    Title = sendNotificationEntity.Title,
                    ImageLink = sendNotificationEntity.ImageLink,
                    Summary = sendNotificationEntity.Summary,
                    Author = sendNotificationEntity.Author,
                    ButtonTitle = sendNotificationEntity.ButtonTitle,
                    ButtonLink = sendNotificationEntity.ButtonLink,
                    Buttons = sendNotificationEntity.Buttons,
                    CreatedBy = sendNotificationEntity.CreatedBy,
                    CreatedDate = sendNotificationEntity.CreatedDate,
                    SentDate = sendNotificationEntity.SentDate,
                    IsDraft = sendNotificationEntity.IsDraft,
                    IsImportant = sendNotificationEntity.IsImportant,
                    IsScheduled = sendNotificationEntity.IsScheduled,
                    ScheduledDate = sendNotificationEntity.ScheduledDate,
                    Teams = sendNotificationEntity.Teams,
                    Rosters = sendNotificationEntity.Rosters,
                    Groups = sendNotificationEntity.Groups,
                    AllUsers = sendNotificationEntity.AllUsers,
                    MessageVersion = sendNotificationEntity.MessageVersion,
                    Succeeded = 0,
                    Failed = 0,
                    Throttled = 0,
                    TotalMessageCount = sendNotificationEntity.Succeeded,
                    SendingStartedDate = DateTime.UtcNow,
                    Status = NotificationStatus.Queued.ToString(),
                    TemplateType = sendNotificationEntity.TemplateType,
                    SendTypeId = sendNotificationEntity.SendTypeId,
                    TenantId = sendNotificationEntity.TenantId,
                    AdaptiveCardContent = sendNotificationEntity.AdaptiveCardContent,
                    EmailTitle = sendNotificationEntity.EmailTitle,
                    EmailBody = sendNotificationEntity.EmailBody,
                    CsvLink = sendNotificationEntity.CsvLink,
                    AdditionalFileLink = sendNotificationEntity.AdditionalFileLink,
                    TenantName = sendNotificationEntity.TenantName,
                    AuthorTeamId = sendNotificationEntity.AuthorTeamId,
                    AuthorTeamName = sendNotificationEntity.AuthorTeamName,
                    AuthorChannelId = sendNotificationEntity.AuthorChannelId,
                    AuthorChannelName = sendNotificationEntity.AuthorChannelName,
                };
                await this.CreateOrUpdateAsync(sentNotificationEntity);
                ...
                // await this.DeleteAsync(draftNotificationEntity);
                return sendNotificationEntity.RowKey;
            }
            catch (Exception ex)
            {
                this.Logger.LogError(ex, ex.Message);
        ...
        /// <inheritdoc/>
        public async Task UpdateRecallNotificationStatusAsync(string notificationId, NotificationStatus status)
        {
            var notificationDataEntity = await this.GetAsync(
        ...
        /// <inheritdoc/>
        public async Task SaveRecallExceptionInNotificationDataEntityAsync(
            string notificationDataEntityId,
            string errorMessage)
        {
            var notificationDataEntity = await this.GetAsync(
            ...
            if (notificationDataEntity != null)
            {
                ...
                // Set the end date as current date.
                notificationDataEntity.RecalledDate = DateTime.UtcNow;
        ...
        /// <inheritdoc/>
        public async Task<IEnumerable<NotificationDataEntity>> GetAllRecallSentNotificationsAsync()
        {
            var result = await this.GetAllAsync(NotificationDataTableNames.RecallSentNotificationsPartition);
