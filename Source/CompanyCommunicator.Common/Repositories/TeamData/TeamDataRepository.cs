...
namespace Microsoft.Teams.Apps.CompanyCommunicator.Common.Repositories.TeamData
{
    ...
    public class TeamDataRepository : BaseRepository<TeamDataEntity>, ITeamDataRepository
    {
        ...
        /// <inheritdoc/>
        public async Task<IEnumerable<TeamDataEntity>> GetAllSortedAlphabeticallyByNameAsync(string tenantId)
        {
            string strFilter = TableQuery.GenerateFilterCondition("TenantId", QueryComparisons.Equal, tenantId);
            var teamDataEntities = await this.GetWithFilterAsync(strFilter, TeamDataTableNames.TeamDataPartition);
            var sortedSet = new SortedSet<TeamDataEntity>(teamDataEntities, new TeamDataEntityComparer());
            return sortedSet;
        }
